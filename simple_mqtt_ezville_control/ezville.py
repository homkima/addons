...
def ezville_loop(config):
                    ...
                    if STATE_PACKET or ACK_PACKET:
                        ...
                        if MSG_CACHE.get(packet[0:10]) != packet[10:] or FORCE_UPDATE:
                                ...
                                if STATE_PACKET:
                                    ...
                                    for id in range(1, spc + 1):
                                        ...
                                        if discovery_name not in DISCOVERY_LIST:
                                            ...
                                            for payload_template in DISCOVERY_PAYLOAD[
                                                name
                                            ]:
                            ...
                            # 일괄차단기 ACK PACKET은 상태 업데이트에 반영하지 않음
                                ...
                                await update_state(
                                    name, "outing", rid, sbc, outingonoff
                                )
    ...
    async def mqtt_discovery(payload):
        ...
        # Discovery에 등록
        topic = "homeassistant/{}/ezville_wallpad/{}/config".format(
            intg, payload["name"]
        )
                ...
                if device == "thermostat":
                    if topics[2] == "power":
                        ...
                        sendcmd = checksum(
                            ...
                            + f"1{idx}"
                            + RS485_DEVICE[device]["target"]["cmd"]
                            + "01"
                            + "{:02X}".format(value)
                            + "0000"
                        )
        ...
        for i in range(CMD_RETRY_COUNT):
            ...
            if debug:
                log(
                    ...
                    + send_data["statcmd"][1]
                    + ", Current: "
                    + DEVICE_STATE.get(send_data["statcmd"][0])
                )
            ...
            # 이후에는 정해진 간격 혹은 Random Backoff 시간 간격을 주고 ACK 확인
                ...
                if RANDOM_BACKOFF:
                    await asyncio.sleep(
                        random.randint(0, int(CMD_INTERVAL * 1000)) / 1000
                    )
                else:
    ...
    def initiate_socket():
        ...
        while True:
            try:
                ...
                log(
                    "[ERROR] Server에서 연결을 거부합니다. 재시도 예정 (" + str(retry_count) + "회 재시도)"
                )
                time.sleep(1)
                retry_count += 1
                continue
